...
@Component({
  selector: 'app-notes-liste',
  templateUrl: './notes-liste.component.html',
  styleUrls: ['./notes-liste.component.css']
})
export class NotesListeComponent implements OnInit{
  ...
  public listeModulesCours: any;
  // Single List
  public singleModule: any;
  ...
  getAllDepartments() {
   ...
   onDepartementChange(): void {
    ...
    getAllModulesCours(codeCours: string) {
      this.listeModulesCours = [];
      const url = apiConfig.admin.module.getAllModsByCours(codeCours);
      // use it here
      this.AdminService.getResourceMany(url, {}).subscribe(
        (data) => {
          this.listeModulesCours = data.body;
          console.log(this.listeModulesCours);
        },
        (err) => {
          console.log('erreur', err.error.message);
        }
    ...
    getSingleUEAndModule(){
      var formValue = this.form.controls;
      this.singleModule = formValue['module'].value || '';
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.getSingleUEAndModule();

      // variables of forms
     const codeEC: string = formValue['module'].value || '';
     ...
     const parcours: string = formValue['parcours'].value || '';
      ...
      // List PV of student
      const url = apiConfig.admin.notes.getListNotesEC(
        anneeAca,
        codeEva,
        codeEC,
        parcours
  ...
  update(item:any) {
   ...
   const codeUE: string = formValue['cours'].value || '';
    if(codeEC!= null &&  codeEva !=null &&  anneeAca != null &&  parcours != null ){
      const encodedId = encodeURIComponent(codeEC+"%"+codeUE+"%"+parcours+"%"+anneeAca+"%"+item.matricule+"%"+item.nom+"%"+codeEva+"%"+item.valeur+"%"+item.id);
    
      this.router.navigate(['notes/updatenoteec/', encodedId]);
    }
