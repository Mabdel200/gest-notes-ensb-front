import { Component, OnInit } from '@angular/core';
import { FormBuilder, Validators } from '@angular/forms';
import { Router, ActivatedRoute } from '@angular/router';
import { ServicesService } from '../../core/services/services.service';
import { read, utils } from 'xlsx';
import { api as apiConfig } from '../../core/configs/constants';

@Component({
  selector: 'app-module-form-multiple',
  templateUrl: './module-form-multiple.component.html',
  styleUrls: ['./module-form-multiple.component.css']
})
export class ModuleFormMultipleComponent implements OnInit {

  module: any;
  moduleJson: any = [];
  cours: any = [];
  parmodule: any = [];
  credit: any = [];
  formmodule: any;
  submitted = false;
  userId: any;
  file?: File;

  constructor(
    private fb: FormBuilder,
    private AdminService: ServicesService,
    private router: Router,
    private route: ActivatedRoute) { }

  ngOnInit(): void {
    this.getAllCredits();
    this.getAllCours();
    this.getAllModule();
  }


  getAllModule() {
    this.parmodule = [];
    const url = `${apiConfig.admin.module.getAll}`;
    this.AdminService.getResources(url).subscribe(
      (data) => {
        this.parmodule = data.body;
      },
      (err) => {
        console.log('erreur', err.error.message);
      }
    );
  }


  getAllCours() {
    this.cours = [];
    const url = `${apiConfig.admin.cours.getAll}`;
    this.AdminService.getResources(url).subscribe(
      (data) => {
        this.cours = data.body;
      },
      (err) => {
        console.log('erreur', err.error.message);
      }
    );
  }


  getAllCredits() {
    this.credit = [];
    const url = `${apiConfig.admin.credit.getAll}`;
    this.AdminService.getResources(url).subscribe(
      (data) => {
        this.credit = data.body;
      },
      (err) => {
        console.log('erreur', err.error.message);
      }
    );
  }



  onSelectFile(events: any) {
    if (events.target.files && events.target.files[0]) {
      const reader = new FileReader();
      reader.readAsArrayBuffer(events.target.files[0]);
      reader.onload = (e) => {
        const data = e?.target?.result;
        const workbook = read(data, { type: "array" });
        const sheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[sheetName];
        let json = utils.sheet_to_json(worksheet);
        this.moduleJson = json
        // console.log(json);
        //this.json = json;
      };

    }
  }


  uploadFileData(event: MouseEvent) {
    this.submitted = true;
    const formData = new FormData();

    console.log(this.moduleJson);

    const propertiesArray = ['code', 'natureUE', 'intitule', 'cours', 'credit'];
    this.moduleJson.forEach((item: any) => {
      // console.log(Object.keys(item).toString().toLowerCase());
      for (const prop of Object.keys(item)) {
        if (propertiesArray.includes(prop.toLowerCase())) {
          this.cours.forEach((i: any) => {
            if (i.nom === item.cours) {
              formData.append('cours_id', i.id);
            }
          });

          this.credit.forEach((i: any) => {
            if (i.nom === item.credit) {
              formData.append('credit_id', i.id);
            }
          });



          formData.append('code', item.code);
          formData.append('natureUE', item.natureUE);
          formData.append('intitule', item.intitule);

          // console.log(formData.getAll);

        }
      }


      // Save datas from files in database
      const url = `${apiConfig.admin.module.create}`;
      this.AdminService.saveResource(url, formData).subscribe(
        (data) => {
          const res: any = data;
          console.log('response: ', res);
          console.log('ID: ', res.body.id);
          if (res.status === 201) {
          } else {
            this.submitted = false;
            console.log('echec');

          }
        },
        (err) => {
          // this.notify.error(Errors(err.error.message), 'Major Error');
          console.log('erreur', err.error.message);
          // console.log(Errors(err.error.message));
        }
      );
    })

  }
}
