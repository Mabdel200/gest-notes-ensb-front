...
@Component({
  selector: 'app-pv-semestre',
  templateUrl: './pv-semestre.component.html',
  styleUrl: './pv-semestre.component.scss'
})
export class PvSemestreComponent implements OnInit{
 ...
 onDepartementChange(): void {
  ...
  submit() {
    if (this.form.valid) {
      ...
      // variables of forms
     const cycle: number = formValue['cycle'].value ? +formValue['cycle'].value : 0;
     const anneeAca: number = formValue['annee'].value|| '';
     const parcours: string = formValue['parcours'].value || '';

      // List PV of student
      const url = apiConfig.admin.notes.getPVSemestre(
        anneeAca,
        cycle,
        parcours
      );
