...
@Component({
  selector: 'app-pv-ec',
  templateUrl: './pv-ec.component.html',
  styleUrl: './pv-ec.component.scss'
})
export class PvEcComponent implements OnInit{
 ...
 onDepartementChange(): void {
  ...
  ngOnInit(): void {
    ...
    // Change State for UE et Parcours
  ...
  submit() {
    if (this.form.valid) {
     ...
     const codeModule: string = formValue['module'].value || '';
    //  const type: any = formValue['type_etudiant'].value || '';
     ...
     const parcours: string = formValue['parcours'].value || '';
      ...
      // List PV of student
      const url = apiConfig.admin.notes.getPVModule(
        anneeAca,
        codeModule,
        parcours
      );
