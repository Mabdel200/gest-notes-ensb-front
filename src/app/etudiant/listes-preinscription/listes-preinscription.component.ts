...
@Component({
  selector: 'app-listes-preinscription',
  templateUrl: './listes-preinscription.component.html',
  styleUrl: './listes-preinscription.component.scss'
})
export class ListesPreinscriptionComponent  implements OnInit{
 ...
 // Export to Excel
  ...
  update(item: Etudiant ) {
    
    const encodedId = encodeURIComponent(item.dateDeNaissance+"%"+item.email+"%"+item.genre+"%"+item.id+"%"+item.lieuDeNaissance+"%"+item.matricule+"%"+item.nationalite+"%"+item.nom+"%"+item.numeroTelephone+"%"+item.region+"%"+item.type+"%");
    
    this.router.navigate(['etudiant/updatepreinscription/', encodedId]);
  }
}
